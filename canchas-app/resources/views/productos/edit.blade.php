...
<head>
    ...
    <title>Editar Producto - Gestión de Canchas</title>
    ...
    <div class="flex-1 ml-64 flex flex-col min-h-screen">
        ...
        <header class="bg-white h-16 shadow-sm border-b border-gray-100 flex items-center justify-between px-8 sticky top-0 z-20">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 tracking-tight">Editar Producto</h1>
                    <p class="text-xs text-gray-400">{{ $producto->nombre }}</p>
                </div>
            </div>
            <div class="flex items-center gap-3">
                {{-- Botón eliminar --}}
                <form action="{{ route('productos.destroy', $producto) }}" method="POST" onsubmit="return confirm('¿Eliminar {{ $producto->nombre }}? Esta acción no se puede deshacer.')">
                    @csrf @method('DELETE')
        ...
        <main class="flex-1 p-8">
            <div class="max-w-2xl mx-auto space-y-5">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-xl px-5 py-4 flex items-center gap-4 fade-in">
                    ...
                    <div>
                        <p class="text-blue-800 text-sm font-semibold">Estado actual del inventario</p>
                        <p class="text-blue-600 text-xs mt-0.5">
                            ...
                            Valor total: <strong>${{ number_format($producto->precio * $producto->stock, 2, ',', '.') }}</strong>
                        </p>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-in">
                    ...
                    <form action="{{ route('productos.update', $producto) }}" method="POST" class="p-6 space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                            <p class="text-xs text-gray-500 font-medium uppercase tracking-wider mb-1">Nuevo Valor en Inventario</p>
                            <p class="text-2xl font-bold text-gray-800" id="preview-total">
